        ...
        while True:
            ...
            for op in func_map:
                if cur_generation.endswith(op+"("):
                    ...
                    funcmodel.inference_mode = "func_embedding"
                    if return_top > 0:
                        results, token_log = results     #token_log(token, [(token, logits, prob)])
                        logs.append(token_log)
...
def vh_embedding_inference(case_idx, question, funcmodel, temperature, top_p, max_func_call):
    ...
    for _ in range(max_func_call):
        ...
        # inputs = funcmodel.generate([inputs], max_gen_len=1, temperature=temperature, top_p=top_p)[0]
        if inputs.endswith(">"):
...
def kamel_embedding_inference(templates, case_idx, question, funcmodel, temperature, top_p, max_gen_len, max_func_call):
    ...
    if "funcgeneral" not in templates:
        templates["funcgeneral"] = templates["100"]
    try:
        results = []
        func_calls = []
        while True:
            if max_func_call == 0:
                break
            prompt = templates["100"].replace("[QUESTION]", question) + cur_generation
