...
def load(ckpt_dir: str, tokenizer_path: str, local_rank: int, world_size: int, func_dict: dict) -> FunctionLM:
    ...
    # linear = Linear(model, tokenizer, model.params.dim, len(func_dict), bias=False)
    print(f"Loaded in {time.time() - start_time:.2f} seconds")
    return funcmodel

def main(ckpt_dir: str = "llama", tokenizer_path: str = "llama/tokenizer.model", input_file: str = "",lr: float = 1e-4, num_epochs: int = 10, dataset: str = "", log_prefix="", only_functoken=True, log_each=False):
