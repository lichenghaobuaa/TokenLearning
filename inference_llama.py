...
from inference_modes import react_inference, func_embedding_inference, kamel_embedding_inference, vh_embedding_inference
...
def main(ckpt_dir: str, tokenizer_path: str, temperature: float = 0, top_p: float = 0.95, mode: str = "baseline", dataset = "original", return_top: int = 5, logits_bias: float = 0, func_load_path: str = "None", st_idx=0, ed_idx=10000, suffix=""):
    ...
    #
        ...
        for name in os.listdir("icl"):
            file_path = os.path.join("icl", name)
    ...
    for case_idx, question in tqdm(enumerate(test_cases), total=len(test_cases)):
        ...
        if case_idx >= ed_idx:
            break
        if mode == "react_inference":
            log = react_inference(templates, case_idx, question, funcmodel, temperature, top_p, max_gen_len)
